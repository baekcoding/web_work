...
public class CafeDao {
	...
	/*
	 * static 메소드는 생성자를 호출하지 않고 클래스명으로 바로 호출을 하기 떄문에 메소드 호출 전에 무언가 준비 작업을 하고 싶다면
	 * static 블록 안에서 하면 된다 static 블록은 해당 클래스를 최초로 사용할 때 한번만 실행되기 때문에 초기화 작업을 하기에 적당한
	 * 블록이다.
	 */
	static {
	...
	public List<CafeDto> getList(CafeDto dto) {
		...
		try {
			...
			String sql = "SELECT *" + " FROM" + "   (SELECT result1.*, ROWNUM AS rnum" + "   FROM"
					...
					+ "      FROM board_cafe" + "      ORDER BY num DESC) result1)" + " WHERE rnum BETWEEN ?  AND ?";
			pstmt = conn.prepareStatement(sql);
			// ? 에 값 바인딩 할게 있으면 하기
